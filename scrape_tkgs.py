...
# Gerçekçi Chrome tarayıcı başlıkları – WAF/bot korumasını aşmak için
BROWSER_HEADERS = {
    "User-Agent": (
    ...
    "Pragma": "no-cache",
    "Referer": "https://www.turksat.com.tr/tr/tablo",
...
def _create_session() -> requests.Session:
    ...
    retry = Retry(
        total=3,
        backoff_factor=2,          # 0s, 2s, 4s arası bekleme
...
# ---------------------------------------------------------------------------
def scrape_turksat(session: requests.Session) -> list[dict]:
    ...
    Raises ConnectionError / Timeout on network failures.
    """
    print("  Türksat Excel indiriliyor...")
    resp = session.get(TURKSAT_EXCEL_URL, timeout=30)
...
def _load_existing_json(path: Path) -> dict | None:
    """Load and validate existing tkgs_turksat.json as fallback."""
    if not path.exists():
        return None
    try:
...
def main():
    ...
    # Step 1: Download & parse — with WAF/network error handling
    print("\n[1/2] Türksat resmi frekans listesi çekiliyor...")
    try:
        channels = scrape_turksat(session)
        fetch_ok = True
        print(f"  → {len(channels)} FTA TV kanalı bulundu")
    except (ConnectionResetError, ConnectionError, requests.ConnectionError,
            requests.Timeout, requests.HTTPError, OSError) as exc:
        print(f"  ⚠ Türksat sunucusuna bağlanılamadı: {type(exc).__name__}: {exc}")
        print("  ⚠ WAF/ağ engellemesi olabilir – yedek veriye geçiliyor.")

    # Fallback: if fetch failed or returned 0 channels, keep existing JSON
    if not fetch_ok or not channels:
        existing = _load_existing_json(output_path)
        if existing:
            ch_count = existing.get("channel_count", len(existing.get("channels", [])))
            version = existing.get("version", "?")
            print(f"\n📦 Mevcut JSON korunuyor: {output_path}")
            print(f"   Mevcut sürüm: {version}, {ch_count} kanal")
            print("\n✅ Yedek veri ile çıkılıyor (exit 0).")
            sys.exit(0)
        else:
            print("\n⚠ Mevcut JSON bulunamadı veya geçersiz.")
            print("  İlk çalıştırmada Türksat'a erişim şart.")
            print("  Lütfen yerel makineden çalıştırıp JSON'ı commit edin.")
            sys.exit(1)
